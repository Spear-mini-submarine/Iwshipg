...
public class HandleAutoGunItem extends Item {
    private static final int FIRE_RATE = 5; //0.25秒一发
    private static final float MUZZLE_VELOCITY = 5.5F; // 步枪弹初速更高，更平直

    public HandleAutoGunItem(Properties properties) {
        super(properties);
    }
    ...
    public void fire(Level level, Player player, ItemStack gunStack) {
        ...
        if (!activeAmmo.isEmpty() && activeAmmo.getItem() instanceof AutocannonAmmoItem round) {
            ...
            if (projectile != null) {
                ...
                projectile.setChargePower(4.0F);//毁伤被绿
